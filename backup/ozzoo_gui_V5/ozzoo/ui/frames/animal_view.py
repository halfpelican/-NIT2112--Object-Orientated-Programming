...
Can Breed: {"Yes" if animal.can_breed else "No"}"""
    ...
    def _play_feeding_frenzy(self, animal: Animal) -> None:
        ...
        available_food = sorted(self._zoo.food_stock.keys())
        if not available_food:
            available_food = ["eucalyptus", "grass", "meat", "fish", "insects"]
        selected_food = tk.StringVar(value=animal.get_diet())
        ...
        walk_away_limit = 4
        min_target_width = 34.0
        shrink_per_success = 6.0
        ...
        def current_marker_speed() -> float:
            ...
            return min(16.0, 8.0 + (state["successes"] * 0.8))
        ...
        def toss_food() -> None:
            ...
            if not self._zoo.use_feeding_frenzy_attempt():
            ...
            correct_food = selected_food.get().strip().lower() == animal.get_diet().lower()
        ...
        def animate_marker() -> None:
            ...
            dialog.after(35, animate_marker)
