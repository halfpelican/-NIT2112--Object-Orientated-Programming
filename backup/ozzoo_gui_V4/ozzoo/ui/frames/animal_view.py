...
Can Breed: {"Yes" if animal.can_breed else "No"}"""
    ...
    def _play_feeding_frenzy(self, animal: Animal) -> None:
        """Run a lightweight Feeding Frenzy mini-game and boost happiness."""
        if not animal.is_alive:
            self._app.show_warning("Feeding Frenzy", f"{animal.name} is no longer alive.")
            return

        visitors = len(self._zoo.visitors)
        if visitors <= 0:
        ...
        # Each visitor gives one toss attempt.
        attempts = visitors
        avg_satisfaction = self._zoo.average_visitor_satisfaction / 100
        hunger_factor = max(0.0, min(1.0, animal.hunger / 100))
        hit_chance = max(0.25, min(0.9, 0.35 + (avg_satisfaction * 0.35) + (hunger_factor * 0.2)))

        hits = 0
        misses = 0
        preference_meter = 0
        miss_limit = max(2, round(attempts * 0.4))

        for _ in range(attempts):
            if random.random() < hit_chance:
                hits += 1
                # Better timing fills preference meter faster.
                preference_meter += 20 if random.random() < 0.35 else 12
            else:
                misses += 1
                if misses >= miss_limit:
                    break

        completed_attempts = max(1, hits + misses)
        preference_meter = min(100, preference_meter)
        walked_away = misses >= miss_limit
        hit_ratio = hits / completed_attempts

        stars = max(1, min(5, round(hit_ratio * 5)))
        stars_display = ("⭐" * stars) + ("☆" * (5 - stars))

        happiness_gain = round(6 + (hit_ratio * 14) + (preference_meter / 100 * 8))
        if walked_away:
            happiness_gain = max(0, happiness_gain - 5)
        happiness_gain = min(25, happiness_gain)
        ...
        result_lines = [
            f"{animal.name} completed Feeding Frenzy!",
            "",
            f"Visitors: {visitors}",
            f"Toss attempts used: {completed_attempts}/{attempts}",
            f"Successful tosses: {hits}",
            f"Preference meter: {preference_meter}%",
            f"Rating: {stars} out of 5 stars {stars_display}",
            f"Happiness: {old_happiness}% → {animal.happiness}% (+{actual_gain})",
        ]
        if walked_away:
            result_lines.append("The animal walked away early after too many misses.")
