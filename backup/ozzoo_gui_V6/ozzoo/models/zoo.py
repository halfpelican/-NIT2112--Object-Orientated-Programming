...
class Zoo:
    ...
    def update_mission_progress(self) -> None:
        ...
        if self._current_mission.mission_type == MissionType.KEEP_HAPPY:
            ...
            happy_animals = [
                ...
                if a.is_alive and not a.is_sick and a.species == target_type and a.happiness >= 70
            ]
    ...
    def complete_mission(self) -> tuple[bool, str, List[str], List[str]]:
        """
    ...
    def advance_day(self) -> Dict[str, Any]:
        ...
        if self._current_mission:
            ...
            if self._current_mission.is_complete:
                success, msg, mission_births, breeding_events = self.complete_mission()
                if success:
                    report["events"].append(msg)
                    if mission_births:
                        report["births"].extend(mission_births)
                    if breeding_events:
                        report["events"].extend(breeding_events)
        ...
        # Tick mission timer (days remaining) at end of day
        if self._current_mission:
