...
class RingtailPossum(Mammal):
    """
    Australian Ringtail Possum - small arboreal herbivore.

    Ringtail possums are tree-dwelling marsupials that feed mainly on leaves
    and flowers. They are agile climbers and spend most of their time in
    canopy habitats.
    """

    def __init__(self, name: str, age: int, gender: str = "unknown") -> None:
        """
        Initialise a new Ringtail Possum.

        Args:
            name: The possum's given name.
            age: Age in years (must be >= 0).
            gender: "male", "female", or "unknown".
        """
        super().__init__(
            name,
            age,
            species="RingtailPossum",
            fur_colour="grey-brown",
            gender=gender,
        )
        self._tail_grip_strength = 70

    @property
    def tail_grip_strength(self) -> int:
        """Return the possum's prehensile-tail grip strength (0-100)."""
        return self._tail_grip_strength

    def make_sound(self) -> str:
        """Return the sound a ringtail possum makes."""
        return f"{self._name} makes a soft chattering trill."

    def get_diet(self) -> str:
        """Return the ringtail possum's diet type."""
        return "eucalyptus"

    def get_habitat_type(self) -> str:
        """Return the primary habitat type for ringtail possums."""
        return "eucalyptus_grove"

    def climb(self) -> str:
        """Describe ringtail possum climbing behaviour."""
        return f"{self._name} climbs quickly through branches with its curled tail."
    ...
    def to_dict(self) -> dict:
        ...
        Returns:
            A dictionary containing all ringtail possum attributes for persistence.
        """
        data = super().to_dict()
        data["class"] = "RingtailPossum"
        data["tail_grip_strength"] = self._tail_grip_strength
        return data
