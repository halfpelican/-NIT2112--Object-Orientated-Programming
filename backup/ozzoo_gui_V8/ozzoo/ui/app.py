...
class OzZooApp(tk.Tk):
    ...
    def __init__(self, zoo: Optional[Zoo] = None):
        """
        Initialise the main Tk application shell.

        Args:
            zoo: Optional preloaded zoo instance to bind at startup.
        """
        super().__init__()
        ...
        # Populate header day/budget when launching with an existing zoo.
    ...
    def _create_header(self) -> tk.Frame:
        ...
        # Day/Budget display (right side)
        self._info_frame = tk.Frame(header, bg=self.COLOURS["primary"])
        self._info_frame.pack(side=tk.RIGHT, padx=20)
        ...
        self._budget_label.pack(anchor="e")
    ...
    def update_header_info(self) -> None:
        """Update day and budget details shown in the header."""
        if not self._zoo:
            self._day_label.config(text="📅 Day: -")
            self._budget_label.config(text="💰 Budget: $0.00")
            return

        self._day_label.config(text=f"📅 Day: {self._zoo.day}")
