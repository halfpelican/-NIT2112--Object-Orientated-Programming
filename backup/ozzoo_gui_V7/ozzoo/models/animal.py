...
class Animal(ABC):
    ...
    SICK_HEALTH_DECAY = 8
    SICK_HAPPINESS_DECAY = 10
    SICK_AUTO_RECOVERY_DAYS = 3
    SICK_RECOVERY_HEALTH_COST = 10
    ...
    def health(self, value: int) -> None:
        ...
        # Critically low health can trigger sickness state
    ...
    def is_sick(self) -> bool:
        """Check if this animal is currently sick."""
    ...
    def daily_update(self) -> None:
        ...
        - If sick, health/happiness decay faster
        ...
        # Sick animals deteriorate faster and can auto-recover over time
        if self._is_sick:
            self._sick_days += 1
            self.health = self._health - self.SICK_HEALTH_DECAY
            self.happiness = self._happiness - self.SICK_HAPPINESS_DECAY

            if self._is_alive and self._sick_days >= self.SICK_AUTO_RECOVERY_DAYS:
                # Natural recovery is possible but comes with a significant health cost
                self.health = self._health - self.SICK_RECOVERY_HEALTH_COST
                if self._is_alive:
                    self._is_sick = False
                    self._sick_days = 0
                    self._notify_observers("recovered")
    ...
    def become_sick(self) -> bool:
        """
        Put the animal into a sick state.

        Returns:
            True if sickness state changed, False if already sick or not alive.
        """
        if not self._is_alive or self._is_sick:
            return False
    ...
    def cure_sickness(self) -> bool:
        """
        Cure the animal's sickness.

        Returns:
            True if the animal was cured, False if not sick or not alive.
        """
        if not self._is_alive or not self._is_sick:
            return False
    ...
    def from_dict(cls, data: dict) -> Animal:
        ...
        instance._is_sick = data.get("is_sick", False)
        instance._sick_days = data.get("sick_days", 0)
